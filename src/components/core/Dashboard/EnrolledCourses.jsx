...
export default function EnrolledCourses() {
  ...
  return (
    <>
      <div className="text-3xl text-richblack-800 font-semibold">Enrolled Courses</div>
      {!enrolledCourses ? (
        ...
        <div className="my-8 text-richblack-5">
          ...
          {enrolledCourses.map((course, i, arr) => (
            <div
              className={`flex items-center border border-richblack-200 ${
                i === arr.length - 1 ? "rounded-b-lg" : "rounded-none"
              }`}
              key={i}
            >
              <div
                className="flex w-[45%] cursor-pointer items-center gap-4 px-5 py-3 text-richblack-600"
                onClick={() => {
                  navigate(
